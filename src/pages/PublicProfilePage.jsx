...
import {
  ...
  SkillsSection,
  ContactSection,
} from "../components/public/sections.jsx";
...
function activeSections(data) {
  ...
  if (data.certifications?.length || data.achievements?.length) list.push("certifications");
  ...
  if (state.status === "not_found") {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-slate-950 px-6 text-center text-slate-300">
        <h1 className="text-3xl font-extrabold text-white">Profile not found</h1>
        <p className="mt-3 text-sm text-slate-400">
          No public profile for "{username}". Check the link or ask the owner to share again.
        </p>
      </div>
    );
  }
